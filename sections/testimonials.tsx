      ...
      <div className="section-shell relative">
        ...
        <div className="no-scrollbar mt-14 flex gap-5 overflow-x-auto pb-4">
          {testimonials.map((testimonial, index) => (
            ...
            >
              <div
                className="flex gap-1 text-sand-400"
                aria-label={`${testimonial.stars} ${text.testimonials.stars}`}
              >
                {Array.from({ length: testimonial.stars }).map((_, star) => (
                  <Star key={star} className="h-4 w-4 fill-current" />
                ))}
              </div>
              <p className="text-safe mt-6 leading-8 text-white/74">
                “{localize(testimonial.text, language)}”
              </p>
              <div className="mt-7 flex items-center gap-3">
                ...
                <div className="min-w-0">
                  <p className="text-safe font-semibold">{testimonial.name}</p>
                  <p className="text-safe text-sm text-white/50">
                    {localize(testimonial.place, language)}
                  </p>
                </div>
              </div>
            </motion.article>
